...
void RenderSystem::DrawPainter(const ur::Device& dev, ur::Context& ctx, const ur::RenderState& rs,
...
void RenderSystem::DrawTexQuad(const ur::Device& dev, ur::Context& ctx, const ur::RenderState& rs,
...
void RenderSystem::DrawTexture(const ur::Device& dev, ur::Context& ctx, const ur::RenderState& rs,
                               const ur::TexturePtr& tex, const sm::rect& pos,
...
void RenderSystem::DrawTexture(const ur::Device& dev, ur::Context& ctx, const ur::RenderState& rs,
                               int tex_w, int tex_h, const ur::TexturePtr& tex, const sm::rect& pos, const sm::Matrix2D& mat, bool use_dtex)
{
	float vertices[8];
	CalcVertices(pos, mat, vertices);

	auto draw_without_dtex = [&](std::shared_ptr<rp::IRenderer>& rd, const float* vertices, const ur::TexturePtr& tex)
	{
...
void RenderSystem::DrawTexture(const ur::Device& dev, ur::Context& ctx,
...
void RenderSystem::DrawColor(const ur::Device& dev, ur::Context& ctx,
                             const std::shared_ptr<ur::ShaderProgram>& shader, const sm::mat4& mat)
{
...
void RenderSystem::DrawText(ur::Context& ctx, const std::string& text, const Textbox& style,
